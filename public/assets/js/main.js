...
function makeInviteButton(socket_id) {
    let newHTML = "<button type='button' class='btn btn-info'>Invite</button>";
    let newNode = $(newHTML);
    newNode.click(() => {
        let payload = {
...
socket.on('game_update', (payload) => {
    ...
    if (my_color === 'light') {
        $("#my_color").html('<h3 id="my_color">You are the light player</h3>');
    }
    else if (my_color === 'dark') {
        $("#my_color").html('<h3 id="my_color">You are the dark player</h3>');
    }
    else {
        $("#my_color").html('<h3 id="my_color">Your color is unknown</h3>');
    }

    if (payload.game.whose_turn === 'light') {
        $("#my_color").append('<h4>Turn: Light</4>');
    }
    else if (payload.game.whose_turn === 'dark') {
        $("#my_color").append('<h4>Turn: Dark</4>');
    }
    else {
        $("#my_color").append('<h4>Error: Unknown Turn</4>');
    }
